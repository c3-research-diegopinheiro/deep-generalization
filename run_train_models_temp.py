...
from builders.image_generator_builder_temp import get_train_generator, get_test_generator
...
def run(data_x, data_y, model_config):
    ...
    for train_index, test_index in kf.split(data_x, data_y):
        ...
        try:
            train_gen, val_gen = get_train_generator(data_x, data_y, train_index)
            curr_model = model_builder.train_model_for_dataset(model_config, train_gen, val_gen)
            rw.write_model(curr_model, f'train_mixed')
            ...
            for noise_amount_testing in NOISE_LEVELS:
                ...
                rw.write_metrics_results(
                    f'train_mixed_test_{noise_amount_testing}',
                    'mixed',
